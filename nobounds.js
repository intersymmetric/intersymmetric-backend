...
const certApplication = "api.intersymmetric.xyz/";

if (env == "live") {
  const privateKey = fs.readFileSync(
    certStorage + certApplication + "api.intersymmetric.xyz.key",
    "utf8"
  );
  const certificate = fs.readFileSync(
    certStorage + certApplication + "api.intersymmetric.xyz.crt",
    "utf8"
  );
...
let blankGrid = new Array(numInstruments).fill(new Array(numSteps).fill(false));
  ...
  socket.on("roomJoin", (room) => {
    ...
    if (users[socket.id] !== room) {
      ...
      if (rooms.hasOwnProperty(room)) {
        ...
        velocity[room] = 1.0;
        length[room] = 1.0;
        (maxCells[room] = 32), (prevInsertions[room] = []);
